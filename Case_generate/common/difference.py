        ...
        if sheet_ori.name == sheet_tar.name:
            ...
            if sheet_ori.name == sub_name:
                ...
                for rows in range(1, sheet_ori.nrows):
                    origin_list = sheet_ori.row_values(rows)  # 源表i行数据
                    target_list = sheet_tar.row_values(rows)  # 目标表i行数据
                    origin_xls[rows] = origin_list  # 源表写入字典
...
if __name__ == '__main__':
    for i in ['前台api', '后台api']:
        diff_excel(r'D:\Tester\Scripts\Case_generate\data_old\demo_api.xlsx',
                   r'D:\Tester\Scripts\Case_generate\data_new\demo_api.xlsx', '{}'.format(i))
        diff_json(r'D:\Tester\Scripts\Case_generate\data_old\前台api_data.json',
                  r'D:\Tester\Scripts\Case_generate\data_new\前台api_data.json', '')
